-- I want to copy some code from GHC, and it often uses this module.
-- Copied directly from GHC, and slightly modified to fit the architecture
-- differences.
...
module Utils.Misc (
        ...
        isSingleton, only, expectOnly, Utils.Misc.singleton,
...
expectOnly _   (a:_) = a
#endif
expectOnly msg _     = panic ("expectOnly: " <> T.pack msg)
...
expectNonEmptyPanic msg = panic ("expectNonEmpty: " <> T.pack msg)
...
minWith get_key xs = assert (not (null xs))
  partialhead (sortWith get_key xs)
  where
    partialhead []    = panic "minWith"
    partialhead (x:_) = x
