-- Specialised finite maps, for things with @Unique@s.
--
-- This module is copied directly from GHC.Types.Unique.FM,
-- with minor changes to fit our architecture.
-- Several functions in this module refer to "unique determinism,"
-- this is a concern in GHC but is not something that we are currently
-- concerned with, and the reference notes do not exist in our
-- compiler. Feel free to look at GHC sources if you are interested.
...
Basically, the things need to be in class @HasUnique@, and we use the
...
unitUFM :: HasUnique key => key -> elt -> UniqFM key elt
...
zipToUFM :: HasUnique key => [key] -> [elt] -> UniqFM key elt
...
listToUFM :: HasUnique key => [(key,elt)] -> UniqFM key elt
...
listToIdentityUFM :: HasUnique key => [key] -> UniqFM key key
...
addToUFM :: HasUnique key => UniqFM key elt -> key -> elt  -> UniqFM key elt
addToUFM (UFM m) k v = UFM (M.insert (getKey $ getUnique k) v m)

addListToUFM :: HasUnique key => UniqFM key elt -> [(key,elt)] -> UniqFM key elt
...
adjustUFM :: HasUnique key => (elt -> elt) -> UniqFM key elt -> key -> UniqFM key elt
...
delFromUFM :: HasUnique key => UniqFM key elt -> key    -> UniqFM key elt
delFromUFM (UFM m) k = UFM (M.delete (getKey $ getUnique k) m)

delListFromUFM :: HasUnique key => UniqFM key elt -> [key] -> UniqFM key elt
...
nonDetFoldWithKeyUFM f z (UFM m) = M.foldrWithKey f' z m
  where
    f' k e a = f (unsafeReconstructUnique k) e a
...
mapMaybeWithKeyUFM f (UFM m) = UFM (M.mapMaybeWithKey (f . unsafeReconstructUnique) m)

mapUFM_Directly :: (Unique -> elt1 -> elt2) -> UniqFM key elt1 -> UniqFM key elt2
mapUFM_Directly f (UFM m) = UFM (M.mapWithKey (f . unsafeReconstructUnique) m)
...
filterUFM_Directly p (UFM m) = UFM (M.filterWithKey (p . unsafeReconstructUnique) m)
...
elemUFM :: HasUnique key => key -> UniqFM key elt -> Bool
...
lookupUFM :: HasUnique key => UniqFM key elt -> key -> Maybe elt
...
lookupWithDefaultUFM :: HasUnique key => UniqFM key elt -> elt -> key -> elt
...
nonDetKeysUFM (UFM m) = map unsafeReconstructUnique $ M.keys m
...
nonDetStrictFoldUFM_DirectlyM f z0 (UFM xs) = M.foldrWithKey c return xs z0
  ...
  where c u x k z = f (unsafeReconstructUnique u) z x >>= k
        {-# INLINE c #-}

nonDetStrictFoldUFM_Directly:: (Unique -> elt -> a -> a) -> a -> UniqFM key elt -> a
nonDetStrictFoldUFM_Directly k z (UFM m) = M.foldlWithKey' (\z' i x -> k (unsafeReconstructUnique i) x z') z m
...
nonDetUFMToList (UFM m) = map (\(k, v) -> (unsafeReconstructUnique k, v)) $ M.toList m
...
pprUniqFM :: (a -> Doc) -> UniqFM key a -> Doc
...
pprUFM :: UniqFM key a -- ^ The things to be pretty printed
       -> ([a] -> Doc) -- ^ The pretty printing function to use on the elements
       -> Doc          -- ^ 'SDoc' where the things have been pretty
                       -- printed
...
pprUFMWithKeys
       ...
       -> ([(Unique, a)] -> Doc) -- ^ The pretty printing function to use on the elements
       -> Doc                    -- ^ 'SDoc' where the things have been pretty
                                 -- printed
